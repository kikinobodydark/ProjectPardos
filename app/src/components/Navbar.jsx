    ...
    >
      ...
      <div className="d-flex align-items-center gap-3">
        {activeCompany && (
          <div 
            ref={companyRef}
            className={`d-flex align-items-center px-3 py-2 rounded-3 position-relative select-none ${userRole === 'admin' && companies.length > 0 ? 'cursor-pointer' : ''}`}
            style={{ 
              backgroundColor: 'var(--bg-surface)', 
              border: '1px solid var(--border-color)', 
              minHeight: '54px',
              transition: 'all 0.2s ease',
              cursor: userRole === 'admin' && companies.length > 0 ? 'pointer' : 'default'
            }}
            onClick={() => {
